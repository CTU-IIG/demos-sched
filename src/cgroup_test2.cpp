...
int main()
{
    ...
    if (pid == 0) { // launch new process
        ...
        if (execl("../build/infinite_proc", "../build/infinite_proc", "100000", "hello", NULL) ==
            -1)
            err(1, "execv");
    } else {
        // wait until process in cgroup
        sleep(1); // TODO ev callback on fd_procs, read cg.procs and check if it
                  // is there?
        ...
        // check empty cgroup (empty cgroups/procs, notify_on_release) (v2
        // cgroup.events (0 empty))
        // TODO
