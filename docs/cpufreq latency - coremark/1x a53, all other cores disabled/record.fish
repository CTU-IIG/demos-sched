...
for i in (seq $BASE_I (math $BASE_I + $REPETITION_COUNT - 1))
    for freq_i in 0 1 2 3
        ...
        echo $name
        /demos/demos-sched -p imx8_fixed:$freq_i,0 -C "
...
for i in (seq $BASE_I (math $BASE_I + $REPETITION_COUNT - 1))
    for freq_i in 1 2 3
        ...
        set cmd "$CMD_BASE >""$name"".out"
        echo $name
        /demos/demos-sched -p imx8_alternating:0,$freq_i,0,0 -C "
            windows: [{length: $LENGTH, slices: [{cpu: $CPUS, sc_partition: SC1}]}]
            partitions: [{name: SC1, processes: [{budget: $LENGTH, cmd: '$cmd'}]}]
        "
    end
...
for i in (seq $BASE_I (math $BASE_I + $REPETITION_COUNT - 1))
    for freq_i in 0 1 2
        ...
        set cmd "$CMD_BASE >""$name"".out"
        echo $name
        /demos/demos-sched -p imx8_alternating:3,$freq_i,0,0 -C "
            windows: [{length: $LENGTH, slices: [{cpu: $CPUS, sc_partition: SC1}]}]
            partitions: [{name: SC1, processes: [{budget: $LENGTH, cmd: '$cmd'}]}]
        "
    end
